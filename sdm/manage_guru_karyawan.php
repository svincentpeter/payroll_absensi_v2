...
$strataConfig  = getStrataConfig();            // fungsi di mgk_salary_handler.php
...
if (isset($_GET['ajax']) && $_GET['ajax'] == '1') {
    ...
    switch ($_POST['case'] ?? '') {
        ...
        case 'LoadingGuru':          LoadingGuru($conn);              break;
        case 'CreateGuru':           CreateGuru($conn);               break;
        case 'GetGuruDetail':        send_response(0, GetGuruDetail($conn)); break;
        case 'UpdateGuru':           UpdateGuru($conn);               break;
        case 'DeleteGuru':           DeleteGuru($conn);               break;

        // --- Pengaturan gaji (fungsi ada di mgk_salary_handler.php) ---
        case 'update_gaji_pokok':            updateGajiPokok($conn);            break;
        case 'update_gaji_strata_guru':      updateGajiStrataGuru($conn);       break;
        case 'update_gaji_strata_karyawan':  updateGajiStrataKaryawan($conn);   break;
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" style="color: #000;">
        ...
        <table class="table table-sm mb-4">
          ...
          <tr id="detailLamaKontrakRow"><th>Lama Kontrak</th><td id="detailLamaKontrak"></td></tr>
          <tr id="detailTglSelesaiRow"><th>Tanggal Selesai</th><td id="detailTglSelesai"></td></tr>
          <tr><th>Pendidikan</th><td id="detailPendidikan"></td></tr>
          <tr><th>Strata</th><td id="detailStrata"></td></tr>
          <tr id="detailGajiPokokRow"><th>Gaji Pokok</th><td id="detailGajiPokok"></td></tr>
          <tr id="detailSalaryIndexRow"><th>Salary Index</th><td id="detailSalaryLevel"></td></tr>
        </table>
        ...
        <table class="table table-sm mb-2">
          ...
          <tr><th>Status Perkawinan</th><td id="detailStatusPerkawinan"></td></tr>
...
function getBadgeRole(role) {
    switch ((role||'').trim()) {
        case 'P':  return '<span class="badge" style="background:#007bff;color:#fff;">Pendidik</span>';
        case 'TK': return '<span class="badge" style="background:#17a2b8;color:#212529;">Tenaga Kependidikan</span>';
        case 'M':  return '<span class="badge" style="background:#dc3545;color:#fff;">Manajerial</span>';
...
$(document).ready(function(){
    ...
    let currentPage=1, pageSize=10, baseUrl="<?= getBaseUrl(); ?>";
    loadGuru(1);
...
$('#modalEdit').on('shown.bs.modal', function() {
        ...
        for(let i=1;i<=pages;i++){
            let li=$('<li>').addClass('page-item'+(i===currentPage?' active':''))
                  .append($('<a>').addClass('page-link').attr('href','#').text(i)
                    .click(e=>{e.preventDefault(); loadGuru(i);}));
            ul.append(li);
        }
    }
    ...
    $('#addJoinStart,#addLamaKontrak').change(()=>{ toggleKontrak('add'); previewTglSelesai('add'); });
    $('#editJoinStart,#editLamaKontrak').change(()=>{ toggleKontrak('edit'); previewTglSelesai('edit'); });
    $('#modalAdd').on('shown.bs.modal',()=>previewTglSelesai('add'));
    $('#modalEdit').on('shown.bs.modal',()=>{ toggleKontrak('edit'); previewTglSelesai('edit'); });

    // --- [3.8] AUTO HITUNG USIA ---
    function autoCalcAge(dobSel, ageSel){
        let dob=$(dobSel).val();
        if(!dob){ $(ageSel).val(''); return; }
        let b=new Date(dob), t=new Date(), age=t.getFullYear()-b.getFullYear(),
    ...
    $(document).on('click','.btn-view',function(){
        let id=$(this).data('id');
        $.ajax({
            url:"manage_guru_karyawan.php?ajax=1", type:"POST",
            ...
            error:()=>{ $('#loadingSpinner').hide(); showToast('Terjadi kesalahan saat mengambil detail.','error'); }
        });
    });
$(document).on('click', '.btn-edit', function () {
    ...
    $.ajax({
        ...
        error: () => { $('#loadingSpinner').hide(); showToast('Gagal mengambil detail.', 'error'); }
    });
});
    ...
    $('#delete-guru-form').submit(function(e){
        e.preventDefault();
        let id=$('#delId').val(); if(!id){ return showToast('ID tidak ditemukan.','error'); }
        let f=$(this);
        $.ajax({
            url:"manage_guru_karyawan.php?ajax=1", type:"POST",
    ...
    $('#edit-guru-form').submit(function(e){
        ...
        if(!formEl.checkValidity()){
            e.stopPropagation(); $(formEl).addClass('was-validated'); return;
        }
    ...
    $('#gaji-strata-form-guru').submit(function(e){
        e.preventDefault();
        let f=$(this);
        if(!this.checkValidity()){ e.stopPropagation(); f.addClass('was-validated'); return; }
        $.ajax({
            url:"manage_guru_karyawan.php?ajax=1",
            type:"POST", data:f.serialize(),
            dataType:"json",
            beforeSend:()=>{
                f.find('button[type="submit"]').prop('disabled',true);
                f.find('.spinner-border').removeClass('d-none');
