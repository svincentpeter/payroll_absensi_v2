...
define('JENIS_ABSENSI', [
    'Normal', 'Izin', 'Sakit', 'Cuti', 'Bolos', 'Libur', 'Lembur'
]);
...
function map_status_kehadiran(string $jenis): string {
    switch (strtolower($jenis)) {
        case 'bolos': return 'tanpa_keterangan';
        case 'izin': case 'sakit': case 'cuti': case 'libur':
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['draw'])) {
        $columns = [
            'a.id','a.tanggal','a.jadwal','a.jam_kerja','a.valid','a.pin',
            'a.nip','a.nama','a.departemen','a.lembur','a.jam_masuk',
            'a.scan_masuk','a.terlambat','a.scan_istirahat_1','a.scan_istirahat_2',
            'a.jam_pulang','a.scan_pulang','a.jenis_absensi'
        ];
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            switch (strtolower($row['jenis_absensi'])) {
                case 'izin': $badgeJenis='badge-info';    break;
                case 'sakit':$badgeJenis='badge-warning'; break;
                case 'cuti': $badgeJenis='badge-primary'; break;
                case 'bolos':$badgeJenis='badge-danger';  break;
                case 'libur':$badgeJenis='badge-success'; break;
                case 'lembur':$badgeJenis='badge-info';   break;
            }
            $nested = [
                $no++,
    ...
    if (($_POST['action'] ?? '') === 'update') {
        ...
        if ($id_absensi <= 0) {
            $_SESSION['notif_error']="ID tidak valid."; header("Location:koreksi_absensi.php"); exit;
        }
        $tanggal=$_POST['tanggal']??'';
        if (!validate_date($tanggal)) {
            $_SESSION['notif_error']="Format tanggal tidak valid."; header("Location:koreksi_absensi.php"); exit;
        }
        foreach (['jam_masuk','scan_masuk','scan_istirahat_1','scan_istirahat_2','jam_pulang','scan_pulang'] as $f){
            if (!empty($_POST[$f]) && !validate_time($_POST[$f])){
                $_SESSION['notif_error']="Format waktu $f tidak valid."; header("Location:koreksi_absensi.php"); exit;
            }
        }
        ...
        $stmt->bind_param(
            'sssissssississssssi',
            $tanggal,$jadwal,$jam_kerja,$valid,$pin,$nip,$nama,$departemen,
            $lembur,$jam_masuk,$scan_masuk_dt,$terlambat,$ist1_dt,
            $ist2_dt,$_POST['jam_pulang'],$scan_pulang_dt,$jenis_absensi,
            $status_kehad,$id_absensi
        );
        if ($stmt->execute()) {
            ...
            $c->execute(); $c->bind_result($id_anggota); $c->fetch(); $c->close();
            $bl = date('n',strtotime($tanggal)); $th = date('Y',strtotime($tanggal));
            $u1 = $conn->prepare("CALL UpdateRekapAbsensi(?,?,?)"); $u1->bind_param('iii',$id_anggota,$bl,$th); $u1->execute(); $u1->close();
            $u2 = $conn->prepare("CALL UpdateRekapMingguan(?,?)"); $u2->bind_param('ss',$tanggal,$tanggal); $u2->execute(); $u2->close();
            $_SESSION['notif_success']="ID $id_absensi berhasil dikoreksi.";
        } else {
            $_SESSION['notif_error']="Gagal update: ".$conn->error;
        }
        header("Location:koreksi_absensi.php"); exit;
    }

    // 4c. Delete Absensi
    if (($_POST['action'] ?? '') === 'delete') {
        $id_absensi = (int)($_POST['id_absensi']??0);
        if ($id_absensi<=0) { $_SESSION['notif_error']="ID tidak valid."; header("Location:koreksi_absensi.php"); exit; }
        log_before_delete($conn,$id_absensi,$_SESSION['nip']);
        $st0 = $conn->prepare("SELECT id_anggota,tanggal FROM absensi WHERE id=?");
        $st0->bind_param('i',$id_absensi); $st0->execute(); $st0->bind_result($id_anggota,$tanggal); $st0->fetch(); $st0->close();
...
.page-title i {
    ...
    font-size: 2.8rem;
        ...
        #absensiTable th, #absensiTable td { text-align:center; vertical-align:middle; white-space:nowrap;}
        .ui-autocomplete { z-index:1050!important; }
        #calendar-container { max-width:600px; margin:0 auto; }
        .fc .fc-day-top, .fc .fc-day-number, .fc .fc-event-title,
        .fc .fc-event, .fc-toolbar h2, .fc-day-header { color:#000!important; }
...
<h1 class="page-title">
       ...
       Koreksi Absensi
                    ...
                    <script>
                        document.addEventListener("DOMContentLoaded",function(){
                            Swal.fire({icon:'success',title:'Sukses',text:<?= json_encode($_SESSION['notif_success']); ?>, timer:3000, showConfirmButton:false});
                        });
                    </script>
                    ...
                    <script>
                        document.addEventListener("DOMContentLoaded",function(){
                            Swal.fire({icon:'error',title:'Error',text:<?= json_encode($_SESSION['notif_error']); ?>, timer:3000, showConfirmButton:false});
                        });
                    </script>
                    <?php unset($_SESSION['notif_error']); endif; ?>
                    ...
                    <!-- Kalender -->
                    <div class="mb-4" id="calendar-container"><div id="calendar"></div></div>
                            ...
                            <table id="absensiTable" class="table table-sm table-bordered table-striped display nowrap" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Tanggal</th><th>Jadwal</th><th>Jam Kerja</th><th>Valid</th>
                                        <th>PIN</th><th>NIP</th><th>Nama</th><th>Departemen</th><th>Lembur</th>
                                        <th>Jam Masuk</th><th>Scan Masuk</th><th>Terlambat</th>
                                        <th>Scan Istirahat 1</th><th>Scan Istirahat 2</th>
                                        <th>Jam Pulang</th><th>Scan Pulang</th><th>Jenis Absensi</th><th>Aksi</th>
                                    </tr>
                                </thead>
                            </table>
                        </div>
    ...
    $(function(){
        ...
        var table = $('#absensiTable').DataTable({
            ...
            ajax: {
                ...
                data: {
                    ...
                    departemen: "<?= htmlspecialchars($departemen); ?>",
            ...
            dom: 'Bfrtip',
            buttons: [
                { extend:'copyHtml5', className:'btn btn-secondary btn-sm', text:'<i class="fas fa-copy"></i> Copy' },
                ...
                { extend:'colvis', className:'btn btn-warning btn-sm', text:'<i class="fas fa-columns"></i> Kolom' }
            ],
        ...
        $('.autocomplete-nama').autocomplete({ source:namaK, minLength:2 });

        // Modal Edit populate
        $('#modalEdit').on('show.bs.modal', function(e){
            var btn = $(e.relatedTarget), m = $(this);
        ...
        $('form').on('submit', function(){
            var b=$(this).find('button[type="submit"]'); b.prop('disabled', true); b.find('.spinner-border').removeClass('d-none');
        });
    });
    </script>
</body>
