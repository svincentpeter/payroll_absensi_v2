...
if (!function_exists('ProcessPayroll')) {
    function ProcessPayroll($conn)
    {
        ...
        SELECT id 
         ...
         LIMIT 1
    ")->fetch_assoc();
    $phInc = intval($phIncRow['id'] ?? 0);

        // 1) Hitung total earnings & deductions dari employee_payheads (draft/revisi, excl rapel/id 7)
       ...
       SELECT jenis, SUM(amount) AS total
        ...
        GROUP BY jenis
   ");
   $stmtSum->bind_param("ii", $id_anggota, $phInc);
        $stmtSum->execute();
        ...
        // 4) Ambil data anggota (gaji pokok & salary_index)
        ...
        $no_rekening       = $conn->real_escape_string($empData['no_rekening']);
        $salaryIndexAmount = 0;
        ...
        // 7) Hapus draft lama & status revisi
        ...
        // 8) INSERT PAYROLL header (draft finalisasi keuangan)
        $sqlInsert = "
            INSERT INTO payroll
            ...
            VALUES (
              ...
              '{$tglEsc}', {$basicSalary},
              {$salaryIndexAmount}, {$totalEarnings},
              {$totalDeductions}, {$honorJamLebih},
              {$potonganAbsensi}, {$gajiBersih},
              '{$no_rekening}', '', 'draft'
            )
        ";
        ...
        // 9) Copy detail employee_payheads ke payroll_detail
        $stmtPH = $conn->prepare("
       SELECT id_payhead, jenis, amount
        ...
        WHERE id_anggota = ?
          ...
          AND id_payhead <> ?
   ");
   $stmtPH->bind_param("ii", $id_anggota, $phInc);
        $stmtPH->execute();
        ...
        $stmtDet->close();
            ...
            // 10) Insert annual increments dari kenaikan_gaji_tahunan
    // 10a) Ambil ID payhead ANNUAL_INC (satu kali saja)
    $phIncRow = $conn->query("
        SELECT id
          FROM payheads
         WHERE code = 'ANNUAL_INC'
    ...
    // 10b) Tentukan range periode payroll
    $firstOfMonth = "{$tahun}-" . str_pad($bulan, 2, '0', STR_PAD_LEFT) . "-01";
    $lastOfMonth  = date('Y-m-d', strtotime("{$firstOfMonth} +1 month -1 day"));

    // 10c) Masukkan semua kenaikan aktif sekaligus
    $stmtInc = $conn->prepare("
        INSERT INTO payroll_detail
          (id_payroll, id_anggota, id_payhead, jenis, amount, ref_kgt)
        SELECT
          ?, k.id_anggota, ph.id, 'earnings', k.jumlah, k.id
        FROM kenaikan_gaji_tahunan k
        JOIN payheads ph
          ON ph.code = 'ANNUAL_INC'
        WHERE k.id_anggota      = ?
          AND k.status          = 'aktif'
          AND k.tanggal_mulai  <= ?
    ...
    $stmtInc->close();
        ...
        // 11) Audit log & notifikasi ke keuangan
        $user_nip = $_SESSION['nip'] ?? '';
        add_audit_log($conn, $user_nip, 'ProcessPayroll', "Payroll draft anggota={$id_anggota}");
        if ($_SESSION['role'] === 'sdm') {
            $resNotif = $conn->query("
                SELECT id FROM anggota_sekolah
                 WHERE role IN ('keuangan','superadmin')
            ");
            while ($u = $resNotif->fetch_assoc()) {
                $msg = "Payroll draft untuk anggota {$id_anggota} sudah siap direview.";
                $stmtN = $conn->prepare("
                    INSERT INTO notifications
                      (user_id, message, is_read, role_target)
                    VALUES (?, ?, 0, ?)
                ");
                $roleT = 'keuangan';
...
if (!function_exists('CheckPayrollCompletion')) {
    function CheckPayrollCompletion($conn)
    {
        ...
        add_audit_log($conn, $_SESSION['nip'] ?? '', 'CheckPayrollCompletion', "Bulan={$bulan}, Tahun={$tahun}");
        send_response(0, ['complete' => empty($messages), 'messages' => $messages]);
    }
}

// Hapus file payroll draft
if (!function_exists('RemovePayrollFile')) {
    function RemovePayrollFile($conn)
    {
        ...
        if ($r && !empty($r['support_doc_path'])) {
            if (file_exists($r['support_doc_path'])) @unlink($r['support_doc_path']);
            $up = $conn->prepare("
                UPDATE employee_payheads
                   SET support_doc_path = NULL
                 WHERE id_anggota = ?
                   AND id_payhead = ?
                   AND status      = 'draft'
            ");
