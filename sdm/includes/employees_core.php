        ...
        while ($row = $res->fetch_assoc()) {
            ...
            $data[] = [
                ...
                'badge_unit'          => ($row['jenjang'] === 'UMUM' && trim($row['unit_penempatan']) !== '')
                                          ? "<span class=\"badge\" style=\"background:#0097A7;color:#fff;\">"
                                            . htmlspecialchars($row['unit_penempatan'])
                                            . "</span>"
                                          : "",
...
if (!function_exists('ViewEmployeeDetail')) {
    function ViewEmployeeDetail($conn)
    {
       ...
       LEFT JOIN salary_indices si ON a.salary_index_id = si.id
        ...
        // Format masa kerja dan hitung gaji
        $masaKerja = trim(
            ($emp['masa_kerja_tahun'] > 0 ? $emp['masa_kerja_tahun'] . ' Tahun ' : '')
        ...
        // 2. Ambil payroll draft dan detail draft jika ada
        $payrollDraft        = null;
        $payrollDetailDraft  = [];
            ...
            if ($resDraft->num_rows > 0) {
                ...
                $stmtPD = $conn->prepare("
                    SELECT d.*, p.nama_payhead, p.jenis AS jenis_payhead
                     ...
                     WHERE d.id_payroll = ?
                ");
                $stmtPD->bind_param("i", $payrollDraft['id']);
                if ($stmtPD->execute()) {
        ...
        // 3. Ambil payheads (employee_payheads)
        $stmtPH = $conn->prepare("
        ...
        while ($rw = $resPH->fetch_assoc()) {
            if (!$includeRapel && intval($rw['is_rapel']) === 1) continue;
            $assigned[] = [
        ...
        // 4. Ambil Kenaikan Gaji Tahunan Aktif
        ...
        $kenaikanGajiAktif = $resKG->num_rows > 0
            ? $resKG->fetch_assoc()
            : null;
        $stmtKG->close();
        ...
        // 5. Audit log
        $user_nip   = $_SESSION['nip'] ?? '';
        $detailsLog = "ViewEmployeeDetail: ID=$id, Month=$selectedMonth, Year=$selectedYear";
        ...
        send_response(0, [
            ...
            'salary_index_base'      => floatval($emp['salary_index_base'] ?? 0),
