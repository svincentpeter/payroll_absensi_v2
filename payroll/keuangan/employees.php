...
header("Content-Security-Policy: default-src 'self'; 
    script-src 'self' https://code.jquery.com https://cdnjs.cloudflare.com https://cdn.datatables.net 'nonce-$nonce'; 
...
function LoadingEmployees($conn) {
    ...
    $sql = "SELECT SQL_CALC_FOUND_ROWS id, nip, nama, jenjang, job_title, status_kerja, no_rekening, email, masa_kerja_tahun, masa_kerja_bulan, gaji_pokok 
            FROM anggota_sekolah WHERE 1=1";
    $params = [];
    $types = "";
    ...
    if (!empty($search)) {
        $sql .= " AND (id LIKE ? OR nip LIKE ? OR nama LIKE ? OR jenjang LIKE ? OR job_title LIKE ? OR status_kerja LIKE ? OR no_rekening LIKE ? OR email LIKE ?)";
        $searchParam = "%" . $search . "%";
        for ($i = 0; $i < 8; $i++) {
            $params[] = $searchParam;
            $types .= "s";
    ...
    if (isset($_POST['order'][0]['column']) && isset($_POST['columns'])) {
        ...
        $allowedColumns = ['id', 'nip', 'nama', 'jenjang', 'job_title', 'status_kerja', 'no_rekening', 'email'];
    ...
    $stmtTotal = $conn->query("SELECT COUNT(*) AS total FROM anggota_sekolah");
    $rowTotal = $stmtTotal->fetch_assoc();
    $totalData = $rowTotal['total'];
    $stmtTotal->close();

    // Total data setelah filter
    $stmtFiltered = $conn->query("SELECT FOUND_ROWS() AS total");
    $rowFiltered = $stmtFiltered->fetch_assoc();
    ...
    <a class="dropdown-item btnSelectMonth" href="javascript:void(0)" data-id="' . htmlspecialchars($row['id']) . '">
      <i class="fa fa-calendar"></i> Select Month
        ...
        $data[] = [
            ...
            "status_kerja" => htmlspecialchars($row['status_kerja']),
            "masa_kerja" => $masaKerja,
            "gaji_pokok" => 'Rp ' . $gajiPokok,
...
function AssignPayheadsToEmployee($conn) {
    ...
    try {
        // Hapus dulu payheads lama untuk karyawan
        ...
        foreach ($payheads as $payhead_id) {
            ...
            $nilai = isset($pay_amounts[$payhead_id]) ? floatval($pay_amounts[$payhead_id]) : 0.0;
            if ($nilai < 0) {
...
<body id="page-top">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="row text-center">
              <?php
              // Membuat grid bulan: 2 bulan sebelum sampai 14 bulan ke depan
              $months = [];
              $years  = [];
              $currentYear = date('Y');
              $currentMonth = date('n');
    ...
    $(document).ready(function() {
        ...
        $('#btnApplyFilter').on('click', function(){
            // Contoh pencatatan audit log (jika diinginkan)
            $.ajax({
                url: 'employees.php?ajax=1',
                type: 'POST',
                data: {
            ...
            $.ajax({
                ...
                success:function(resp){
                    if(resp.code === 0){
                        ...
                        $('#detailJenisKelamin').text(e.jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan');
                                    ...
                                    assignedPayheads.forEach(function(ap){
                                        ...
                                        var payheadAmount = `
                                            <div class="payhead-item">
                                                ...
                                                <input type="number" name="pay_amounts[${payheadId}]" class="form-control" value="${ap.amount}" min="0" step="0.01" required>
                                            </div>
                                        `;
                ...
                if($('#selected_payheads option[value="' + payheadId + '"]').length === 0){
                    ...
                    var payheadAmount = `
                        <div class="payhead-item">
                            ...
                            <input type="number" name="pay_amounts[${payheadId}]" class="form-control" value="0" min="0" step="0.01" required>
                        </div>
                    `;
                    $('#selected_payamount').append(payheadAmount);
                    $(this).remove();
