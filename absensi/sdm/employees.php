...
";
    ...
    $detailsLog = "SDM memproses payroll => draft, anggota ID = $id_anggota, oleh $user_nip.";
    add_audit_log($conn, $user_nip, 'ProcessPayroll', $detailsLog);

    // 7) Notifikasi ke keuangan & superadmin jika diproses oleh SDM
...
function AssignPayheadsToEmployee($conn) {
    ...
    // Siapkan statement untuk cek master payheads (akan dipakai dalam loop)
    ...
    // Karena kolom status di-hardcode 'draft', total placeholder untuk INSERT adalah 11.
    ...
    try {
        ...
        foreach ($payheads as $pid) {
            // Inisialisasi variabel untuk iterasi ini
            $pidInt = intval($pid);
            ...
            if ($alreadyExists) {
                ...
                if ($fileBlob !== null) {
                    // Parameter ke-4 (upload_file_blob) – indeks 3 (zero-based)
                ...
                if ($fileBlob !== null) {
                    // Parameter ke-7 (upload_file_blob) – indeks 6 (zero-based)
...
function ViewEmployeeDetail($conn) {
    ...
    // Tambahkan pengambilan payroll_status (berdasarkan bulan & tahun terpilih)
    $selectedMonth = isset($_POST['selectedMonth']) ? intval($_POST['selectedMonth']) : date('n');
    $selectedYear  = isset($_POST['selectedYear']) ? intval($_POST['selectedYear']) : date('Y');
    $stmt = $conn->prepare("
    ...
    if ($res->num_rows > 0) {
        ...
        send_response(0, [
            ...
            'payroll_status'     => $emp['payroll_status'] // tambahkan status payroll
        ]);
    } else {
        $stmt->close();
        send_response(1, 'Anggota tidak ditemukan.');
