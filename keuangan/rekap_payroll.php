...
// Catat audit log saat mengakses halaman
add_audit_log(
    $conn,
    $_SESSION['nip'],
    'AccessPage',
...
// Set nilai default filter (bulan dan tahun saat ini)
$defaultBulan = date('n');
$defaultTahun = date('Y');

/* ------------------------- AJAX HANDLING ------------------------- */
// Hanya menangani request AJAX untuk load data rekap payroll dan audit log
...
/**
 * Fungsi: LoadingRekapPayroll
 * Menampilkan data rekap payroll per jenjang untuk DataTables (tabel utama).
 */
function LoadingRekapPayroll($conn)
{
    // Parameter DataTables dan filter
    ...
    // Jika bulan belum diisi dan show_all tidak aktif, jangan tampilkan data
    if ($bulan <= 0 && !$show_all) {
        $sqlWhereExtra = " AND 0=1 ";
    } else {
        $sqlWhereExtra = "";
    }
    ...
    // Ambil daftar payheads untuk header (earnings terlebih dahulu, lalu deductions)
    $earningPayheads = [];
    ...
    // Buat SELECT dinamis untuk detail payheads
    $dynamicCases = [];
    ...
    // Buat SELECT dinamis outer query
    $outerDynamicSelect = "";
    foreach ($payheads as $ph) {
        $alias = "payhead_" . md5($ph);
    ...
    // LEFT JOIN ke kenaikan_gaji_tahunan untuk kolom "lain-lain"
    $sqlData = "
        SELECT
            a.jenjang,
    ...
    // Sorting tambahan (hanya untuk kolom tertentu)
    ...
    $mapCol['total_lain_lain'] = 'total_lain_lain';
    ...
    while ($row = $resData->fetch_assoc()) {
        error_log("Jenjang dari database: " . $row['jenjang']);
        $rowData = [];
...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800">
                        <i class="fas fa-chart-bar"></i> Rekap Payroll - <?php echo getIndonesianMonthName($defaultBulan) . " " . $defaultTahun; ?>
                    </h1>
                            ...
                            <form id="filterForm" class="row gy-2 gx-3 align-items-center">
                                ...
                                <div class="col-auto">
                                    ...
                                    <select class="form-select" id="filterBulan" name="bulan">
                                        ...
                                        <?php
                                        for ($m = 1; $m <= 12; $m++) {
                                            $selected = ($m == $defaultBulan) ? "selected" : "";
                                    ...
                                    <select class="form-select" id="filterTahun" name="tahun">
                                        ...
                                        if ($stmtTahun) {
                                            ...
                                            while ($row = $resTahun->fetch_assoc()) {
                                                $selected = ($row['tahun'] == $defaultTahun) ? "selected" : "";
                                                echo '<option value="' . htmlspecialchars($row['tahun']) . '" ' . $selected . '>' . htmlspecialchars($row['tahun']) . '</option>';
                                            }
                                            $stmtTahun->close();
                                        }
                                        ?>
    ...
    <script>
        // Siapkan array kolom dinamis berdasarkan payheads (untuk tabel rekap utama)
        var dynamicColumns = [];
    <?php
    foreach ($headerPayheads as $ph) {
        $colKey = "payhead_" . md5($ph);
        // Menambahkan defaultContent pada definisi kolom
        echo "dynamicColumns.push({ data: '$colKey', name: '$colKey', defaultContent: '0' });\n";
    }
    ?>
        ...
        $(document).ready(function() {
            ...
            // Inisialisasi DataTables untuk tabel rekap utama
            ...
            // Karena sekarang aksi detail diubah menjadi link redirect, event untuk modal dihapus.
            // Jadi, hapus event handler untuk .btn-view-rekap-detail dan .btn-view-detailed-payroll
        });
    </script>
</body>
</html>
