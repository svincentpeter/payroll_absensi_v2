...
function LoadingRekapPayroll($conn)
{
  ...
  if (!empty($_POST['order'][0]['column'])) {
    ...
    if ($idx === 0) {
      ...
      $allAliases = array_map(function ($ph) {
        return 'payhead_' . substr(md5($ph), 0, 8);
      }, $GLOBALS['PAYHEADS']);
      $allAliases = array_merge(
        $allAliases,
        array_map(function ($g) {
          return 'grp_' . substr(md5($g), 0, 8);
        }, $GLOBALS['PAYHEAD_GROUPS'])
      );
      $pos = $idx - 2;
  ...
  while ($r = $res->fetch_assoc()) {
    $row = [
      ...
      'total_basic_salary'   => formatNominal($r['total_basic_salary']),
...
<body id="page-top">
  ...
  <script>
  // A) bangun array terpisah sesuai kelompok kolom
  var dynamicEarnings = [];
  <?php foreach ($earningPayheads as $ph):
    $alias = 'payhead_' . substr(md5($ph), 0, 8);
  ?>
    dynamicEarnings.push({
      data: '<?= $alias ?>',
      name: '<?= $alias ?>',
      defaultContent: '0'
    });
  ...
  $(function() {
    ...
    var tbl = $('#rekapPayrollTable').DataTable({
      ...
      // B) susun columns sesuai urutan <th>
      columns: [
        { data: 'jenjang',                name: 'jenjang' },
        { data: 'total_basic_salary',     name: 'total_basic_salary' },
        { data: 'total_salary_index',     name: 'total_salary_index' }
      ]
      .concat(dynamicEarnings)    // semua earning payheads
      .concat(dynamicGroups)      // semua grouped payheads
      .concat([
        { data: 'total_lain_lain',           name: 'total_lain_lain' },
        { data: 'total_potongan_koperasi',   name: 'total_potongan_koperasi' }
      ])
      .concat(dynamicDeductions)  // semua deduction payheads
      .concat([
    ...
    // filter buttons
    $('#btnApplyFilter').on('click', e => { e.preventDefault(); tbl.ajax.reload(); });
    $('#btnResetFilter').on('click', e => {
      e.preventDefault();
      $('#filterForm')[0].reset();
      $('#filterShowAll').val("0");
      tbl.ajax.reload();
